...
public class OnlineSaleStockGUIManager extends AbstractGUIManager implements
		CActionListener, GUIActionListener, ListSelectionListener,
		TableModelListener {
	public OnlineSaleStockGUIManager (String locale){
		setLocale(locale);
		init();
	}
	void init(){
		if (getLocale() != null && !getLocale().equals("")){
			initTemplate(this, "sale/bill/onlineStock.xml", getLocale());
		}else{
			initTemplate(this, "sale/bill/onlineStock.xml");		
			}
		render();
		bindEventHandlers();
