...
public class SearchByWarehouseGUIManager extends AbstractGUIManager implements
		CActionListener, GUIActionListener, ListSelectionListener,
		TableModelListener {
	public SearchByWarehouseGUIManager (String locale){
		setLocale(locale);
		init();
	}
	void init(){
		if (getLocale() != null && !getLocale().equals("")){
			initTemplate(this, "purchase/search/searchByWarehouse.xml", getLocale());
		}else{
			initTemplate(this, "purchase/search/searchByWarehouse.xml");		
			}
		render();
		bindEventHandlers();
	}
