...
public class SearchByCustomerGUIManager extends AbstractGUIManager implements
		CActionListener, GUIActionListener, ListSelectionListener,
		TableModelListener {
	public SearchByCustomerGUIManager (String locale){
		setLocale(locale);
		init();
	}
	void init(){
		if (getLocale() != null && !getLocale().equals("")){
			initTemplate(this, "purchase/search/searchByPOProvider.xml", getLocale());
		}else{
			initTemplate(this, "purchase/search/searchByPOProvider.xml");		
			}
		render();
		bindEventHandlers();
	}
