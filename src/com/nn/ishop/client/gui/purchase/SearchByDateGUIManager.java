...
public class SearchByDateGUIManager extends AbstractGUIManager implements
		CActionListener, GUIActionListener, ListSelectionListener,
		TableModelListener {
	public SearchByDateGUIManager (String locale){
		setLocale(locale);
		init();
	}
	void init(){
		if (getLocale() != null && !getLocale().equals("")){
			initTemplate(this, "purchase/search/searchByDate.xml", getLocale());
		}else{
			initTemplate(this, "purchase/search/searchByDate.xml");		
			}
		render();
		bindEventHandlers();
	}
