...
public class Base64Coder {
	...
	//Mapping character from 6-bit nibbles to base64	 
	private static char[]    map1 = new char[64];
	   static {
	      int i=0;
	      for (char c='A'; c<='Z'; c++) map1[i++] = c; //from A --> Z
	      for (char c='a'; c<='z'; c++) map1[i++] = c;//from a--> z
	      for (char c='0'; c<='9'; c++) map1[i++] = c;//numbers
	      map1[i++] = '+'; map1[i++] = '/'; //sign +, /
	     }
	...
	*/
	public static byte[] decode (char[] in) {
	   ...
	   if (iLen%4 != 0) throw new IllegalArgumentException (
			   "Length of Base64 encoded input string" +
